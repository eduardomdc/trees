...
export function build_tree_geometry (tree : T.pennTree, root_segment : T.Segment) : THREE.BufferGeometry {
    const my_geometry : Geometry = {vertex : [], index : []};
...
function build_segment_geometry (tree : T.pennTree, segment : T.Segment, geometry : Geometry) {
    ...
    if (segment.segment_number == 0) { // first segment in branch
        ...
        //const idx_offsets = cylinder_geometry(geometry, segment.position, segment.rotation, segment.stem.radius, tip, segment.rotation, segment.stem.radius, 6)
        const base_vert_offset = geometry.vertex.length/3;
        const base_points = circle_points(segment.position, segment.rotation, segment.stem.radius, 6)
...
function circle_points (position : THREE.Vector3, orientation : THREE.Quaternion, radius : number, sections : number) : Array<number> {
